...
function printUsage() {
  console.log(
    'Usage: node scripts/share-codec.mjs <encode|encode-json|decode> [value]'
  )
  console.log('')
  console.log('Examples:')
  console.log('  npm run share:encode -- /proc/main()')
  console.log(
    '  npm run share:encode:json -- {"files":{"main.dm":"/proc/main()\\n"}}'
  )
...
async function main() {
  ...
  if (command === 'encode' || command === 'encode-json') {
    ...
    const payload = command === 'encode-json' ? JSON.parse(value) : value

    const encoded = toBase64Url(gzipSync(pack(payload)))
    console.log(encoded)
    return
  }
